...
socket.onmessage = function (e) {
  ...
  if (data.receiver != receiver_id) {
    chat_div = `
    <tr class="max-h-20">    
      <td class="px-4 py-2 bg-green-200 text-black text-left rounded-md flex flex-col max-w-md float-left">
        <p>${data.message}</p>
    ...
    chat_div = `
    <tr class="max-h-20">
      <td  class="px-4 py-2 bg-blue-950 text-white text-right rounded-md flex flex-col max-w-md float-right">
        <p>${data.message}</p>
